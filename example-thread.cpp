...
portMUX_TYPE qam_rx_mux = portMUX_INITIALIZER_UNLOCKED;
...
#ifdef EXAMPLE_THREAD
void IRAM_ATTR qam_rx() {
  ...
  portENTER_CRITICAL_ISR(&qam_rx_mux);
  //portENTER_CRITICAL(&qam_rx_mux);
  ...
  portEXIT_CRITICAL_ISR(&qam_rx_mux);
  //portEXIT_CRITICAL(&qam_rx_mux);
}

void qam_rx_task(void *pvParameters) {
  pinMode(35, INPUT);
  attachInterrupt(digitalPinToInterrupt(35), qam_rx, CHANGE);
  vTaskDelete(NULL);
}
...
void setup() {
  ...
  // Start the qam_rx task on Core 1
  xTaskCreatePinnedToCore(qam_rx_task, "qam_rx_task", 10000, NULL, 1, NULL, 1);
  //attachInterrupt(digitalPinToInterrupt(35), qam_rx, CHANGE);

  // qam_rx_thread.onRun(qam_rx_thread_fn);
  // qam_rx_thread.setStackSize(2048);
  // qam_rx_thread.setPriority(2);
  // qam_rx_thread.start();
